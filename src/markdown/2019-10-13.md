...
このブログは BFF 構成にしたことで SSR 時は API リクエストを叩いていません。しかし、CSR 時は Express に対して Ajax しています。その際に、一度取得したデータを再び取得することがないように、オンメモリでキャッシュする仕組みを備えています。 
...
ブログを作るなら多言語対応したいと思っていたのでしました！ やり方はカンタンで、 [vue-i18n](https://github.com/kazupon/vue-i18n)を使用しただけになります。kazupon さんが作ってくれた OSS ツールですね、感謝です。

使い方は調べてもらえばたくさん日本語記事が出てくるので割愛します。


### インフラに kubenetes, ingress を採用してみた

これはずっと以前から興味を持って少しずつ勉強していたことなのですが、kubenetes を運用してみたいと思っていたので今回の個人開発で使用してみました。
GKE on GCP にデプロイしたのですが、まだまだ kubenetes は初心者なのでほぼデフォルトの設定になっているかと思います。
これから時間を見つけてyamlでの設定を勉強したいなあと思っています。


## まとめ

以上が自分の作ったブログの紹介になります。 何度かブログを開発してきたのですが、毎回「CMS機能を作り込むのに時間を割けられない」という悩みに直面し、そのため記事が書きづらく..という悪循環になっていました。今回はシンプルな実装で、なおかつ GitHub を活用するなど、楽をして品質の良いブログアプリケーションが出来上がったと思っています。
