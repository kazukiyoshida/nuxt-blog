...
2年間の機械学習の経験と 3年のWebシステム開発の経験があります。  
Webシステム開発では25人ほどのエンジニア部門のリーダーを務めました。  
また、戦略コンサルタントとして1年の経験があり、適切な社内調整・資料作成ができます。  

## スキル

### プログラミング言語とフレームワーク
- **Go**: GinによるAPIサーバーの新規開発・本番運用の経験があります。経験年数は1年程です。
  個人的に非常に好きな言語です。gRPCや並行処理についてはプライベートで利用した程度です。
- **Python**: 修士研究で機械学習に用いていました。現在でも簡単なスクリプトやデータ分析は Python で行っています。
  経験年数は7年程です。複数のウェブ・機械学習FWの経験があります。
- **Rust**: 業務での経験はないですが、趣味の開発は主に Rust を使っています。経験年数は1.5年程です。
  個人的に最も好きな言語です。複数のウェブFW、非同期処理、WebAssembly の経験があります。
- **Kotlin**: サーバーの新規開発・本番運用の経験があります。経験年数は 1年弱です。
  ロジックは問題なく組めますが、ライブラリなど技術選定ができるレベルではないと思います。

### クラウド
- **AWS**: 業務では3年程使用していました。趣味の開発ではベーシックな環境を作り、
  ECS Fargate などにデプロイしています。業務で Terraform による環境構築の経験もあります。
- **GCP**: 業務では3ヶ月ほどデータ分析基盤の構築に使用しました。趣味で CloudRun や GKE にサービスをデプロイしたことも
  ありますが、AWS に乗り換えたため最近はあまり使用していません。

### DB・ツールなど
- **MySQL**: 本番環境を含め3年程の経験があります。実行計画をもとに基本的なチューニングが行えます。
- **MongoDB**: 業務での利用はないですが、趣味で半年程の経験があります。簡単な集計などが可能です。
- **Kubernetes**: 業務での利用はないですが、趣味で半年程の経験があります。
...
#### 仮想通貨自動取引システム: （参考: https://kazukiyoshida.net/blog/actor-model-DI）
...
- Qiita: https://qiita.com/_kazuki_yoshida_
